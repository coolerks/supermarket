...
public class InStockServiceImpl implements InStockService {
    ...
    public InStockDetailVO detail(Long id, int pageNum, int pageSize) throws BusinessException {
        ...
        if(supplier==null){
            throw new BusinessException(BusinessCodeEnum.PARAMETER_ERROR,"提供商品方不存在,或已被删除");
        }
        ...
        if(!CollectionUtils.isEmpty(products)) {
            for (Object product : products) {
                ...
                if (dbProduct == null) {
                    ...
                    throw new BusinessException(BusinessCodeEnum.PRODUCT_IS_REMOVE, dbProduct.getName() + "商品已被回收,无法入库");
                } else if(dbProduct.getStatus()==2){
                    throw new BusinessException(BusinessCodeEnum.PRODUCT_WAIT_PASS, dbProduct.getName() + "商品待审核,无法入库");
    ...
    public void publish(Long id) throws BusinessException {
        ...
        if(!CollectionUtils.isEmpty(infoList)){
            for (InStockInfo inStockInfo : infoList) {
                String pNum = inStockInfo.getPNum();//商品编号
                Integer productNumber = inStockInfo.getProductNumber();//入库商品数
                Example o1 = new Example(Product.class);
                o1.createCriteria().andEqualTo("pNum",pNum);
                List<Product> products = productMapper.selectByExample(o1);
                if(products.size()>0){
